    ...
    <div class="px-5 xl:flex-1">
      <div class="max-w-screen-xl mx-auto flex flex-col items-center pt-24 xl:pt-10">
        ...
        <div class="flex flex-col w-full gap-3 mt-4 mb-3">
          <RankingPoints :users="rankings[selectedTab]" />
        </div>

        <!-- Personalny ranking (narazie statyczny) -->
        <div
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40"
          ...
          <path
            ...
            <!-- nakladka svg 32x32 --> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
              viewBox="0 0 32 32" class="absolute top-1/2 left-1/2 w-8 h-8 -translate-x-1/2 -translate-y-1/2">
...
const rankings = ref([[], [], []]) // trzy zakładki: tydzień, miesiąc, cały czas

// funkcja pobierająca ranking dla danego okresu
...
// pobranie danych przy mount
onMounted(async () => {
  rankings.value[0] = await fetchRanking('week')   // tydzień
  rankings.value[1] = await fetchRanking('month')  // miesiąc
  rankings.value[2] = await fetchRanking('all')    // cały czas
})
</script>
