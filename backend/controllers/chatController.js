...
// Pobieranie wszystkich rozmów z informacją o nieprzeczytanych
    ...
    const conversationsWithUnread = await Promise.all(
      conversations.map(async (conv) => {
        const unreadCount = await Message.countDocuments({
          ...
          sender: { $ne: userId }, // wiadomości nie od nas
          createdAt: { 
            $gt: conv.lastRead?.get(userId) || new Date(0) // nowsze niż ostatnie przeczytanie
          }
        });
...
export const handleMessageSent = async (message) => {
  try {
    // Aktualizuj czas ostatniego przeczytania dla nadawcy
